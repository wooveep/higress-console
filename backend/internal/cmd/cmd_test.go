...
func TestResolveRuntimeDependenciesPrefersStructuredEnv(t *testing.T) {
	...
	env := map[string]string{
		...
		"PORTAL_DB_HOST":                          "postgresql-pgpool",
		"PORTAL_DB_PORT":                          "5432",
		"PORTAL_DB_USER":                          "portal",
		"PORTAL_DB_PASSWORD":                      "secret",
		"PORTAL_DB_NAME":                          "aigateway_portal",
		"PORTAL_DB_PARAMS":                        "sslmode=disable",
	...
	require.Equal(t, "host=postgresql-pgpool port=5432 user=portal password=secret dbname=aigateway_portal sslmode=disable", deps.Portal.DSN)
...
func TestResolveRuntimeDependenciesFallsBackToLegacyEnv(t *testing.T) {
	...
	env := map[string]string{
		"HIGRESS_PORTAL_DB_URL":              "jdbc:postgresql://postgresql-pgpool:5432/aigateway_portal?sslmode=disable",
	...
	require.True(t, deps.Portal.Enabled)
	require.Equal(t, "host=postgresql-pgpool port=5432 user=portal password=legacy-secret dbname=aigateway_portal sslmode=disable", deps.Portal.DSN)
	require.True(t, deps.Grafana.Enabled)
	require.Equal(t, "http://legacy-grafana/grafana", deps.Grafana.BaseURL)
}

func TestResolveRuntimeDependenciesNormalizesPostgresParams(t *testing.T) {
	values := runtimeConfigValues{}
	env := map[string]string{
		"PORTAL_DB_HOST":     "postgresql-pgpool",
		"PORTAL_DB_PORT":     "5432",
		...
		"PORTAL_DB_PASSWORD": "secret",
		"PORTAL_DB_NAME":     "aigateway_portal",
		"PORTAL_DB_PARAMS":   "application_name=console",
	}
	...
	require.Equal(
		t,
		"host=postgresql-pgpool port=5432 user=portal password=secret dbname=aigateway_portal application_name=console sslmode=disable",
		deps.Portal.DSN,
	)
}
...
func TestResolveRuntimeDependenciesAllowsExplicitDisable(t *testing.T) {
	...
	env := map[string]string{
		...
		"AIGATEWAY_CONSOLE_PORTALDB_DSN":     "host=db port=5432 user=portal password=secret dbname=manual sslmode=disable",
	...
	require.Equal(t, "host=db port=5432 user=portal password=secret dbname=manual sslmode=disable", deps.Portal.DSN)
...
func TestResolveRuntimeDependenciesSupportsPostgresStructuredEnv(t *testing.T) {
	values := runtimeConfigValues{
		PortalDBEnabled: true,
		PortalDBDriver:  "postgres",
	}
	env := map[string]string{
		"PORTAL_DB_DRIVER":   "postgres",
		"PORTAL_DB_HOST":     "postgresql-pgpool",
		"PORTAL_DB_PORT":     "5432",
		"PORTAL_DB_USER":     "portal",
		"PORTAL_DB_PASSWORD": "secret",
	...
	require.True(t, deps.Portal.Enabled)
	require.Equal(t, "postgres", deps.Portal.Driver)
	require.Equal(t, "host=postgresql-pgpool port=5432 user=portal password=secret dbname=aigateway_portal sslmode=disable", deps.Portal.DSN)
}
